...
public class IDECompiler {
    ...
    /**
     * Compile the source program to LLVM IR and save to a .ll file next to the source.
     * Returns the generated filename (or null on error).
     */
    public String compileToLLVM(String sourceName) {
        SourceFile source = new SourceFile(sourceName);
        Scanner scanner = new Scanner(source);
        report = new IDEReporter();
        Parser parser = new Parser(scanner, report);
        ...
        if (report.numErrors == 0) {
            ...
            if (report.numErrors == 0) {
                ...
                if (res instanceof String) {
                    String llvmText = (String) res;
                    String outName = sourceName.replaceAll("\\.tri$", "") + ".ll";
                    try (BufferedWriter bw = new BufferedWriter(new FileWriter(outName))) {
                        bw.write(llvmText);
                        bw.flush();
                        return outName;
                    } catch (IOException ioe) {
                        System.err.println("Error writing LLVM file: " + ioe.getMessage());
                        return null;
                    }
                }
            }
